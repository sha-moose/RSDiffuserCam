...
import cupy as np
import tv_approx_haar_cp_colour as tv
...
class fista_spectral_numpy():
    def __init__(self, h, mask):
        ...
        self.H = np.fft.fft2((np.fft.ifftshift(self.pad(h), axes = (0,1))), axes = (0,1))[...,np.newaxis][...,np.newaxis]
    ...
    def Hpower(self, x):
        x = np.fft.ifft2(self.H[...,0]* np.fft.fft2(np.expand_dims(x,-1), axes = (0,1)), axes = (0,1))
        x = np.sum(self.mask[...,0]* self.crop(np.real(x)), 2)
        x = self.pad(x)
        return x
    ...
    def run(self, inputs):   
        ...
        # TODO: REMOVE spectral_channels in new version
        xk = np.zeros((self.DIMS0*2, self.DIMS1*2, self.spectral_channels, 3))
        vk = np.zeros((self.DIMS0*2, self.DIMS1*2, self.spectral_channels, 3))
        tk = 1.0
        ...
        for i in range(0,self.iters):
            ...
            if self.show_recon_progress== True and i%self.print_every == 0:
                ...
                fc_img = numpy.max(out_img, axis=-2)
